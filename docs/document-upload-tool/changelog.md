...
## [2026-07-20] — T6 implemented: chunk_strategy support
Model: claude-sonnet-5
Branch: document-upload-tool/t6-chunk-strategy (cut from document-upload-tool/t5-upload-document-tool, pushed, no PR — same gh account-mismatch limitation as T1-T5)
Summary: Added `chunk_strategy: str = "recursive"` to `DocumentConfig` (config/config.py) reading `DOCUMENT_CHUNK_STRATEGY`, with a commented `.env.example` entry. Added `validate_chunk_strategy` to utils/validation.py, rejecting anything outside `{"recursive", "structural"}`. Implemented `structural_chunk(pages, chunk_size, chunk_overlap)` in core/chunking.py: multi-page input passes each page through as one whole chunk (PDF case); single-page input with no page_number splits on Markdown heading lines into whole, un-split sections, falling back to `recursive_chunk` output when there are no headings (`.txt` case, and headingless `.md`). Wired `chunk_strategy: Optional[str] = None` into `upload_document` (tools/document.py), validated before parsing, branching the per-page `recursive_chunk` loop vs. a single whole-document `structural_chunk` call; downstream metadata/embedding/storage/summary/exception-handling unchanged. README updated. All 81 tests pass, including every pre-existing T1-T5 test unmodified — default behavior is unchanged.
Learnings carried forward: `tests/test_config.py` didn't exist yet on the t5 branch (created fresh); a single-page PDF falls into the multi-page passthrough branch of `structural_chunk` (not explicitly called out in the spec, but consistent); added `tests/fixtures/sample_sections.md` (3 headings) since `sample.md` only had one heading.
Manifest: Implementation → confirmed.
